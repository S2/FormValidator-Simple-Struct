package FormValidator::Simple::Struct::AllowDenyStrings;
use 5.008_001;
use strict;
use warnings;
use utf8;
...
use base 'Exporter';
our @EXPORT= qw/ALLOW_STRING DENY_STRING/;

sub ALLOW_STRING{
    my ($s , $allow_strings) = @_;
    for my $allow_string(@$allow_strings){
        return 1 if $s eq $allow_string;
    }
    return 0;
}
...
sub DENY_STRING{
    my ($s , $deny_strings) = @_;
    for my $deny_string(@$deny_strings){
        return 0 if $s eq $deny_string;
    }
    return 1;
}
